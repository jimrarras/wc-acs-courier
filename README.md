...
cp .env.example .env                            # add ACS test-account credentials
vendor/bin/phpunit -c phpunit-integration.xml   # integration tests, real ACS API calls
./build.sh                                      # release zip
```

Manual front-end checks are listed in `docs/testing-checklist-ui.md`.
...
ACS Courier is a trademark of its owner. This plugin is not affiliated with or endorsed by ACS Courier.
